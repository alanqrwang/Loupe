...
loupe_pytorch.utils.add_bool_arg(parser, 'train_cond')
loupe_pytorch.utils.add_bool_arg(parser, 'is_epi')
...
if args.type == 'l':
    ...
    for prefix in args.filename_prefix:
        ...
        models = glob.glob(format_string_filled)
        print(models)
        loss = []
        val_loss = []
        for path in models:
        ...
        plt.plot(loss, label='%s' % (prefix), color=color)
        plt.plot(val_loss, label='%s' % (prefix), color=color, linestyle='dashed')
        plt.grid()
        plt.xlabel('Epoch')
...
if args.type == 'm' or args.type == 's':
    ...
    for i, path in enumerate(paths_to_use):
        ...
        if args.type == 'm' and args.train_cond:
            checkpoint = torch.load(path, map_location=torch.device(args.device))
        ...
        # Slice
            ...
            print(path)
            # if 'regularizer' in path or 'no-dc' in path:
            #     pred, _ = model(torch.tensor(xdata[slice_to_use]).float().unsqueeze(0).to(args.device))
            # else:
            pred, _ = model(torch.tensor(xdata[slice_to_use]).float().unsqueeze(0).to(args.device))
            pred = pred.cpu().detach().numpy()
            im = axes[0, v].imshow(pred[0,...,0], cmap='gray')
            axes[0, v].set_title('%s' % (prefix))
